...
**프롬프트**
> DB가 제대로 생성이 안되었어. DB생성 SQL을 현재 프로그램에 맞게 다시 작성해줘

**작업**
- `"column"` 예약어 처리 포함한 cards 테이블 SQL 재작성 안내
...
**프롬프트**
> 칸반보드를 팀원과 공유해서 사용할 수 있도록 수정할꺼야. 로그인 시 공유코드를 생성할 수 있고, 해당 코드를 다른 사용자가 등록하면 같은 칸반보드를 사용할 수 있도록 해줘. 수정 시 누가 어떤 내용을 수정했는지 이력도 함께 기록하도록 해줘. 작업 내용은 문서에도 최신화해줘

**작업**
- `index.html` — 공유코드 표시/복사, 보드 참여 입력폼, 이력 보기 버튼, 이력 모달 추가
- `app.js` — `initBoard()`, `joinBoard()`, `logHistory()`, `renderHistory()`, Realtime 구독
- `style.css` — 공유코드 배지, 참여 폼, 이력 모달 스타일 추가
- `DatabaseDesign.md` — v3 기준 4개 테이블 스키마 전면 업데이트
- `TASKS.md` — Phase 9 (팀 공유) 추가

---

### 30. 공유코드 유효하지 않음 오류 수정

**프롬프트**
> A 사용자의 공유코드를 B 사용자가 등록 시 유효하지 않다고 오류가 발생해. 원인을 확인하고 수정해줘

**작업**
- 원인: `boards` RLS select 정책이 미가입 사용자의 조회를 차단
- `boards` select 정책에 `auth.role() = 'authenticated'` 조건 추가
- `DatabaseDesign.md` 정책 설명 업데이트

---

### 31. 공유코드 생성 UI 및 보드 선택 기능 개선

**프롬프트**
> 처음 로그인해서 상단에 공유코드 생성 버튼이 있어서 버튼 클릭 시 팝업이 호출되고, 방 이름과 인원을 입력 후 확인시 공유코드를 생성해줘. 해당 공유코드를 다른 사용자와 공유할 수 있고, 하단에는 내가 참여중인 방을 보여줘서 방을 선택할 수 있게 해줘.

**작업**
- `index.html` — `+ 공유코드 생성` 버튼, 생성 모달, 보드 참여 모달, Room Bar 추가
- `app.js` — `initBoard()` 재설계, `switchBoard()`, `renderRoomBar()` 추가
- `style.css` — Room Bar, 생성/참여 모달 스타일 추가
- `boards` 테이블에 `max_members`, `owner_email` 컬럼 추가

---

### 32. Room Bar 코드값 → 방 제목으로 변경

**프롬프트**
> 상단에 참여중인 보드는 코드값이 아닌 방 제목으로 표시해줘

**작업**
- `renderRoomBar()`에서 `share_code` span 제거, 방 이름만 표시
...
**프롬프트**
> 왼쪽에 참여중인 보드에 대한 정보가 있었으면 좋겠어. 보드이름, 생성일자, 생성자에 대한 정보를 보여줘

**작업**
- `index.html` — 좌측 사이드바(`#board-info`) 추가 (보드명, 생성일, 생성자, 역할)
- `app.js` — `renderBoardInfo()` 추가, `switchBoard()` 시 사이드바 업데이트
- `style.css` — `#board-wrapper`, `#board-info` 레이아웃 스타일 추가
- Supabase SQL: `boards.owner_email` 컬럼 추가 안내

---

### 34. 이력 보기 버튼 사이드바 하단으로 이동

**프롬프트**
> 이력보기 버튼을 왼쪽 사이드바 하단으로 옮겨줘

**작업**
- `index.html` — 헤더에서 이력 보기 버튼 제거, 사이드바 하단(`board-info-footer`)으로 이동
- `style.css` — `.board-info-footer` 스타일 추가
...
**프롬프트**
> 문서를 모두 최신화하고 두 저장소 모두 커밋 및 푸시해줘

**작업**
- `TASKS.md` Phase 9 항목 전면 갱신
- `workflow.md` 30~35번 항목 추가
- `CLAUDE.md` 테이블/함수/DOM 구조 v3 기준 업데이트
- 두 저장소 커밋 및 push
